...
export default function EditorElement(props: EditorElementProps) {
  ...
  switch (element.type) {
    ...
    case ElementType.CodeBlock:
      return (
        <code
          className={`block p-2 bg-gray-100 border border-gray-200 rounded ${verticalSpacing} ${className}`}
          {...attributes}
        >
          {children}
        </code>
...
const NoteLinkElement = (props: NoteLinkElementProps) => {
  ...
  const noteLinkClassName = `p-0.25 rounded text-primary-600 cursor-pointer select-none bg-gray-100 hover:bg-gray-200 active:bg-gray-300 ${className} ${
...
const ThematicBreak = (props: ThematicBreakProps) => {
  ...
  return (
    <div className={`py-2 ${className}`} {...attributes}>
      <div
        className={`py-0.25 ${
          selected && focused ? 'bg-blue-100' : 'bg-gray-200'
        }`}
      >
        {children}
      </div>
